...
function renderDetail(block, item, bridge) {
  ...
  const ctaBg = '#ea0063';
  btn.style.cssText = `background:${ctaBg};color:${ctaTextColor(ctaBg)}`;
  if (bridge) {
    btn.addEventListener('click', () => {
      bridge.sendMessage(`Tell me more about ${item.name} cruises with Princess`);
    });
  }
